Shader "UI/HexRimGlow"
{
    // The play-mode gold "ethereal mist" rim glow (UI/CardHoverGlow), adapted to
    // hug a FLAT-TOP HEXAGON instead of a rounded-rect card. Used by the deck
    // builder's Smash-style roster to mark the selected deck.
    //
    ...
    Properties
    {
        ...
        _CardSize     ("Hex cell size (px)", Vector) = (144, 125, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Signed distance to a FLAT-TOP regular hexagon (vertices left/right,
            // horizontal top/bottom edges), r = apothem (centre -> edge middle).
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float  apothem = _CardSize.y * 0.5;          // cell height = 2 * apothem
                float  d = sdHexagon(p, apothem);            // px; <0 inside hex
