Shader "UI/CardEmberAdditive"
{
    // Minimal additive UGUI shader for glowing ember particles. Blend One One so each
    // ember only ADDS light (glow). Respects RectMask2D clipping (_ClipRect).
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex; fixed4 _Color; fixed4 _TextureSampleAdd; float4 _ClipRect; float4 _MainTex_ST;
            ...
            fixed4 frag (v2f IN) : SV_Target
            {
                ...
                #endif
                // premultiply by alpha so additive output = glow * alpha
                return fixed4(c.rgb * a, a);
            }
            ENDCG
        }
    }
