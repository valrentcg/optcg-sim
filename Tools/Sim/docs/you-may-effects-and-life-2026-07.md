...
| 32 | **A PROHIBITION was executed as an INSTRUCTION** — the interpreter matches substrings, so "you cannot draw cards using your own effects during this turn" DREW A CARD. 51 cards carry a prohibition clause; **11 name an action the interpreter can run** (play a Character, add Life to hand, draw). Now refused at the queue chokepoint and logged as "restriction noted" | 51 cards (11 executable) |
