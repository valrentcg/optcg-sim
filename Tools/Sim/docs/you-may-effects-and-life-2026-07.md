...
| 37 | **The 21 undriven heal shapes were a CLAIM, now partly evidence** — `healsweep` drives 63 heal wordings, heals on 42, and wrote the remaining 21 off as "covered by triggerfield/timingsweep — a boundary, not a gap". A cost the fixture cannot pay and a heal that is broken read IDENTICALLY (both "did not heal"), so the write-off could not be trusted. Driving the largest population (`DON!! −N:` heals) from a board that can pay: the heal is CORRECT, lands face-down, and the DON!! is really returned. It also confirmed the engine **prompts** for the DON!! payment rather than auto-paying — which is exactly the decision the brief asks for | EB03-034 + the DON!!-cost heal family |
| 36 | **Seat authority now asserted on the three surfaces this workstream uses** — `opponentdecides` guarded only `resolveChoice`. The Use half (`resolveEffect`), the Skip half (`passEffect`) and the Life look (`deckLookConfirmOrder`) had no equivalent test. All three are correctly gated today; dropping the seat check in `FindPendingEffect` reddens two of the three, so the guard is real and now regression-locked. **No defect — this converts a load-bearing ASSUMPTION behind the UI-gating note into a test** | all optional effects |
| 35 | **Every printing of the two named cards enumerated** — "Nami" is **32 printings, 14 with an optional clause**; "Kalgara" is **4, 1 optional**. Both Kalgaras are covered (OP12-099 = the Life reactive fixed here, OP15-101 = the "you may trash" one on the checklist). Exactly one named card sat in `usevsskip`'s 75-clause inert residual — **OP10-088 Nami** — and driving it from a board that CAN pay proves that entry is fixture mismatch, not defect: compound + alternative + type-filtered cost pays via the Leader branch, via the Stage branch, and correctly refuses with neither | OP10-088 |
| 34 | **Watcher scope, pinned by two untouched members** — a REARRANGE lifts Life cards out and puts them back, and would pay a removal-reactive for free if the watcher read the dip; it does not. "Add 1 card from the top of your Life to your hand" is a third removal route (neither damage nor a paid cost) and does fire it. Also confirmed: card data has **0 corrupted characters**, both circled-digit families (①-⑩ and ➀-➉) are parsed, and no bulleted option line carries a cost or a prohibition that an `^`-anchored regex would miss | rearrange family, EB02-057 |
| 33 | **"Flip life" had only a LEAK check, never a POSITIVE one** — `lifefacing` asks whether any clause leaves a Life card face-up *unasked*, which an entirely unimplemented flip passes trivially (nothing face-up reads as clean). `lifeflip` now asserts the other direction: the "turn 1 card from the top of your Life face-up" COST really flips the top card (and only it), the stack never changes size, and the "if you have a face-up Life card" CONDITION reads the same flag the cost sets. Verified by three separate controls; no defect found — this closes a hole in the EVIDENCE, not in the engine | 4 cost wordings + EB03-051 gate |
| 32 | **A PROHIBITION was executed as an INSTRUCTION** — the interpreter matches substrings, so "you cannot draw cards using your own effects during this turn" DREW A CARD. 51 cards carry a prohibition clause and 11 name a runnable action, but driving all 11 with the guard removed shows **only OP12-099 is actually executed today** — the other 10 name verbs this interpreter does not yet act on. The guard closes 1 live defect and 10 latent ones; `prohibitionsweep` is the net that catches them if the interpreter later learns those verbs (OP02-004/OP06-020 "you cannot add Life cards to your hand" would silently become a heal). Now refused at the queue chokepoint and logged as "restriction noted" | 51 cards (11 executable) |
| 31 | **The Life-removal reactive fired FOUR times for one Life card** — `ApplyCommand` is re-entrant, so every nested command spanning the removal re-fired the new watcher. Depth-guarded to the outermost command | OP11-041, OP12-099, OP08-105 |
| 30 | **The widened watcher paid out the WRONG side** — it fired for both seats without checking whose Life left, so OP08-105 Bonney ("your opponent's Life") drew when HER OWN Life was damaged. The wording is now read: "your or your opponent's" reacts to either side, plain "your opponent's" only to theirs | OP08-105 |
| 29 | **Nami's and Kalgara's "when a card is removed from YOUR or your opponent's Life" never fired for the "your" half** — the reactive was wired only to the battle-damage path, for the ATTACKING player, a documented simplification. Both cards are `[Your Turn]`-gated, and on your own turn the usual way your Life shrinks is PAYING A LIFE COST — so the draw they promise did not happen. Now watched at the command boundary, covering every removal path | OP11-041 Nami, OP12-099 Kalgara |
