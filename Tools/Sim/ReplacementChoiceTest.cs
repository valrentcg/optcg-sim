...
namespace OnePieceTcg.Sim
{
    ...
    public static class ReplacementChoiceTest
    {
        ...
        private static void OffByDefaultTheProtectionStillAutoPays()
        {
            ...
            // "You may" always asks now, so the sweep leaves ONE question per protected victim
            // and nothing is spent until they are answered.
            Check("a sweep queues one question per protected victim and spends nothing yet",
                b.N.CharacterArea.Any(c => c != null && c.InstanceId == victim.InstanceId)
                    && b.N.Life.Count(l => l.FaceUp) == faceUpBefore
                    && b.St.PendingEffects.Count(e => e.Timing == "removalChoice") == 2,
        ...
        private static void OptedInThePlayerCanAcceptAndKeepTheCard()
        {
            ...
            // Only ONE payment is legal per sweep: the first turns the top Life card face-up, and
            // "turn 1 card from the TOP of your Life face-up" cannot be paid again while it stays
            // that way. So the first victim is saved and the second is K.O.'d - correct, not a bug.
            // The suite previously expected two payments because it predated that rule.
            // Which of the two survives depends on the order the questions are answered - Nami
            // guards herself as well - so assert the order-independent invariant instead of naming
            // one of them: exactly one payment was made, and exactly one protected body remains.
            int survivors = b.N.CharacterArea.Count(c => c != null
                && (c.CardId == "ST29-008" || c.CardId == "ST29-009"));
            Check("accepting pays once; the second victim cannot pay and is K.O.'d",
                survivors == 1
                    && b.N.Life.Count(l => l.FaceUp) == faceUpBefore + 1
                    && b.St.PendingEffects.Count == 0,
                $"survivors={survivors} faceUp {faceUpBefore}->{b.N.Life.Count(l => l.FaceUp)} "
